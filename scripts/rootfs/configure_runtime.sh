...
# ---- LED 指示 (实测验证) ----
#   蓝灯 blue:wan: WiFi 关联常亮 (NM dispatcher: wlan0 up/down 事件控制亮度)
#     注意: phy0assoc (mac80211 led trigger) 对 wcn36xx 无效!
#     wcn36xx 是 cfg80211 驱动, 不经过 mac80211 -> ieee80211_led_* 事件永不触发
#     实测: trigger=phy0assoc 时 WiFi 已关联但 brightness=0 (不亮)
#     改用 dispatcher: up->1, down->0 (实测断开/重连验证通过)
#   绿灯 green:wlan: 网络活动闪烁 (netdev tx/rx, 需 ledtrig-netdev 模块)
# local 服务早于 networkmanager, 等待 wlan0 就绪后配置
mkdir -p "${CHROOT}/etc/local.d" "${CHROOT}/etc/NetworkManager/dispatcher.d"
cat > "${CHROOT}/etc/local.d/led-trigger.start" <<'EOF'
#!/bin/sh
# 蓝灯: WiFi 关联常亮; 绿灯: 网络活动闪烁
# 等待 wlan0 就绪 (NM dispatcher 会在 up/down 时控制蓝灯)
i=0
while [ $i -lt 30 ]; do
...
# 蓝灯: trigger 设 none, 亮度交给 NM dispatcher (60-wifi-led.sh)
echo none > /sys/class/leds/blue:wan/trigger 2>/dev/null || true
# 绿灯: 网络活动 (ledtrig-netdev)
echo netdev > /sys/class/leds/green:wlan/trigger 2>/dev/null || true
echo wlan0 > /sys/class/leds/green:wlan/device_name 2>/dev/null || true
echo 1 > /sys/class/leds/green:wlan/tx 2>/dev/null || true
echo 1 > /sys/class/leds/green:wlan/rx 2>/dev/null || true
echo 100 > /sys/class/leds/green:wlan/interval 2>/dev/null || true
EOF
chmod +x "${CHROOT}/etc/local.d/led-trigger.start"
# 蓝灯: WiFi 关联常亮 (NM dispatcher, wcn36xx 无 mac80211 led 事件)
cat > "${CHROOT}/etc/NetworkManager/dispatcher.d/60-wifi-led.sh" <<'EOF'
#!/bin/sh
# WiFi 关联常亮蓝灯 (blue:wan)
# wcn36xx 是 cfg80211 驱动, phy0assoc (mac80211 led trigger) 无效,
# 改用 NM dispatcher: 接口 up/down 事件控制亮度
[ "$1" = "wlan0" ] || exit 0
case "$2" in
    up)   echo 1 > /sys/class/leds/blue:wan/brightness ;;
    down) echo 0 > /sys/class/leds/blue:wan/brightness ;;
esac
EOF
chmod +x "${CHROOT}/etc/NetworkManager/dispatcher.d/60-wifi-led.sh"
# ledtrig-netdev 开机预加载
printf 'ledtrig-netdev\n' >> "${CHROOT}/etc/modules-load.d/containers.conf"
