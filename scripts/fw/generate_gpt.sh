#!/bin/sh -e
# =============================================================================
# fw 子模块: 生成 GPT 分区表 (fastboot 兼容 gpt_both0.bin)
#
# 固定布局 (与可运行版本一致, 仅 boot 扩至 128MB):
#   - boot   128MB (ext2, 内核+initramfs+dtb+extlinux.conf)
#   - rootfs 名义 2015 扇区 (fastboot 刷 sparse 镜像时按实际数据写入,
#             GPT 声明大小不影响刷入, 实际 rootfs.bin ~1.5GB 会完整写入)
# 分区 UUID 固定 (GPT 分区表标识, 与 fstab 用的文件系统 UUID 无关)
# =============================================================================
...
log "Creating GPT partition table (boot=128MB, rootfs=nominal 2015 sectors)..."
...
# 合成 fastboot 兼容分区镜像 (主 GPT + 备份 GPT, 与可运行版本一致的组装方式)
dd if="${TMPDIR}/gpt.img" of=files/gpt_both0.bin bs=512 count=34 2>/dev/null
dd if="${TMPDIR}/gpt.img" bs=512 skip=2 count=32 >> files/gpt_both0.bin 2>/dev/null
dd if="${TMPDIR}/gpt.img" bs=512 skip=481313 >> files/gpt_both0.bin 2>/dev/null
